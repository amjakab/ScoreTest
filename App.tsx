...
const App: React.FC = () => {
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-12 flex flex-col items-center min-h-screen">
      ...
      <section className="relative w-full max-w-md bg-slate-900/50 backdrop-blur-md rounded-[3rem] p-12 border border-slate-700 shadow-2xl mb-12 flex flex-col items-center">
        <div className="absolute -top-6 bg-slate-800 px-6 py-2 rounded-full border border-slate-700 text-xs font-bold text-slate-400 uppercase tracking-widest">
          Scoreboard
        </div>
        
        <div className={`text-9xl font-bangers transition-all duration-500 ${score !== null && score >= 0 ? 'text-emerald-400' : 'text-rose-500'} ${isUpdating ? 'scale-90 opacity-50' : 'scale-100 opacity-100'}`}>
          {score ?? 0}
        </div>
        
        <div className="mt-4 flex items-center gap-2 text-slate-500 text-sm font-mono">
          <span className={`w-2 h-2 rounded-full ${error ? 'bg-rose-500' : isCloud ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`}></span>
          {error ? 'SYNC ERROR' : isCloud ? 'LIVE & REALTIME' : 'LOCAL MODE'}
        </div>
      </section>
